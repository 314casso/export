...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Vessel', fields ['guid']
        db.create_index('nutep_vessel', ['guid'])

        # Adding index on 'Line', fields ['guid']
        db.create_index('nutep_line', ['guid'])

        # Adding index on 'Terminal', fields ['guid']
        db.create_index('nutep_terminal', ['guid'])

        # Adding field 'Contract.guid'
        db.add_column('nutep_contract', 'guid',
                      self.gf('django.db.models.fields.CharField')(max_length=50, null=True, db_index=True),
                      keep_default=False)

        # Adding index on 'Voyage', fields ['guid']
        db.create_index('nutep_voyage', ['guid'])


    def backwards(self, orm):
        # Removing index on 'Voyage', fields ['guid']
        db.delete_index('nutep_voyage', ['guid'])

        # Removing index on 'Terminal', fields ['guid']
        db.delete_index('nutep_terminal', ['guid'])

        # Removing index on 'Line', fields ['guid']
        db.delete_index('nutep_line', ['guid'])

        # Removing index on 'Vessel', fields ['guid']
        db.delete_index('nutep_vessel', ['guid'])

        # Deleting field 'Contract.guid'
        db.delete_column('nutep_contract', 'guid')
    ...
    models = {
        ...
        'nutep.voyage': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '150', 'db_index': 'True'}),
            'vessel': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['nutep.Vessel']", 'null': 'True'})
        }
    }
